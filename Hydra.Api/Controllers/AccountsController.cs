...
public class AccountsController : ControllerBase
{
    ...
    [HttpPost("{accountKey}/activate")]
    [Authorize(Roles = "ADMIN,SUPERADMIN")]
    public async Task<IActionResult> Activate(string accountKey)
    {
        try
        {
            return Ok(await _accountService.ActivateAsync(accountKey));
        }
        catch (Exception exception)
        {
            return BadRequest(Error("ACCOUNT_ACTIVATE_FAILED", exception.Message));
        }
    }
